...
def main() -> int:
    ...
    for size in SIZES:
        ...
        # The genre picker overlay: open, and again filtered by a search.
        app.scene._open_genre_picker()
        render(6)
        shot("menu_genre_picker", size)
        app.scene.genre_picker._on_query("a")
        render(4)
        shot("menu_genre_picker_search", size)
        app.scene.genre_picker._done()
        render(4)
        ...
        app.scene._set_tab("Dict")
        render(6)
        shot("stats_dict", size)
        app.scene._on_search("Dict", "食")
        render(6)
        shot("stats_dict_search", size)
        app.scene._on_search("Dict", "")
