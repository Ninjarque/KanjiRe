...
RELEASED = [
    ...
    "0.13.0", "0.14.0", "0.15.0", "0.16.0", "0.17.0", "0.18.0", "0.19.0",
    "0.20.0", "0.21.0", "0.22.0", "0.23.0", "0.24.0", "0.25.0", "0.26.0",
    "0.27.0", "0.28.0", "0.29.0", "0.29.1", "0.29.2", "0.30.0", "0.31.0",
    "0.31.1", "0.32.0",
]
...
def main(argv=None) -> int:
    ...
    for v in RELEASED:
        ...
        if info is None and v != latest:
            # check_for_update() deliberately reports every benign failure —
            # including a network blip — as "no update", which over ~25
            # sequential requests is enough to fake a stranded version and
            # cry BROKEN CHANNEL at the end of a perfectly good release.
            # One retry tells a real gap apart from a dropped connection.
            time.sleep(1.0)
            info = check_for_update(v)
        if v == latest:
            if info is not None:
